...
In this post, we will use [Symfony Rest Example](https://github.com/hantsy/symfony-rest-sample/t) project as an example, and show how I upgrade to v7 step by step. 
...
Remove `composer.json.lock`, try to run `composer install` command in a terminal to install new packages and rerun the built-in recipes for this project.

## Refresh Codes

There are a few APIs that need to align with the new APIs in Symfony v7. 
...
There is a built-in [UidValueResolver](https://github.com/symfony/symfony/blob/7.0/src/Symfony/Component/HttpKernel/Controller/ArgumentResolver/UidValueResolver.php) can be used to convert the parameter to `Uuidv4`, so we can give up our custom Uuid converter, and switch to use the official one. 
...
Rerun `composer install`, we will something that complain something like *the `getMethods` of `Delete` return type is not matched with the `getMethods` from `Route`: array*. In Symfony v7, all methods and property are require type declaration, it is good to write type safe codes.

Symfony provides a simple scripts to upgrade existing codes and apply types.
...
Finally, we should update the PHP version in Github actions workflow file, drop PHP 8.1 and add PHP 8.3 as build environment. PHP 8.1 is no longer supported in the new Symfony v7.
